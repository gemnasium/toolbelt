...
const (
	SUPPORTED_DEPENDENCY_FILES = `(Gemfile|Gemfile\.lock|gems\.rb|gems\.locked|.*\.gemspec|package\.json|package-lock\.json|npm-shrinkwrap\.json|yarn\.lock|setup\.py|requirements\.txt|requires\.txt|composer\.json|composer\.lock|bower\.json|pom\.xml)$`
)
